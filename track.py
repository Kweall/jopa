                    ...
                    for j, (output) in enumerate(outputs[i]):
                        ...
                        if save_txt:
                            ...
                            with open(txt_path + '.txt', 'a') as f:
                                ...
                                # cursor.execute(f'INSERT INTO b{X}.b{X} ("frame", "id", "bbox_left", "bbox_top", "bbox_w", "bbox_h") VALUES (%s, %s, %s, %s, %s, %s);',
                                #                     (int(frame_idx), int(id), int(bbox_left), int(bbox_top), int(bbox_w), int(bbox_h)))
                                # conn.commit()
                                db = Data_db()
                                db.table_create()
                                db.insert_data(frame_idx, id, bbox_left, bbox_top, bbox_w, bbox_h)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--source', type=str, default='C:/Users/Алексей/PycharmProjects/pobeda/static/uploads/video.mp4', help='source')  # file/folder, 0 for webcam
